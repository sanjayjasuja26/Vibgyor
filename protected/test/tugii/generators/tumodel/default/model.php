...
if (!empty($valid_columns)) :
    ...
    foreach ($valid_columns as $column) :
        ...
        if (preg_match('/(_date)/i', $column->name)) {
            ?>		if ( !isset( $this-><?php echo $column->name ?> )) $this-><?php echo $column->name ?> = date( 'Y-m-d');
            <?php
        }
    endforeach
    ;
    ...
    foreach ($valid_columns as $column) :
        ...
        if (preg_match('/(updated_on|approve_time|update_time)/i', $column->name)) {
            ?>		$this-><?php echo $column->name ?> = date( 'Y-m-d H:i:s');
            <?php
        }
    endforeach
    ;
...
foreach ($columns as $column) :
    ...
    if (strpos($column->name, 'file')) {
        ?>		if(isset($this-><?php echo $column->name ?>))
        $json['<?php echo $column->name ?>'] 	= \Yii::$app->createAbsoluteUrl('<?php echo strtolower($className) ?>/download',array('file'=>$this-><?php echo $column->name ?>));
        <?php
    } else {
        $match = '/^(updated_on|update_time|actual_start|actual_end|password|passcode|activation_key)/i';
        if (!preg_match($match, $column->name)) {
            ?>			$json['<?php echo $column->name ?>'] 	= $this-><?php echo $column->name ?>;
            <?php
        }
    }
    ?>
